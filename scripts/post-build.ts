...
if (process.platform === 'darwin') {
  ...
  const dmgPath = resolve('build', 'PdfStudio.dmg');
  const artifactDir = resolve('artifacts');
  if (existsSync(dmgPath)) {
    if (!existsSync(artifactDir)) mkdirSync(artifactDir, { recursive: true });
    copyFileSync(dmgPath, join(artifactDir, 'macos-arm64-PdfStudio.dmg'));
    console.log('[post-build] Copied PdfStudio.dmg to artifacts/macos-arm64-PdfStudio.dmg');
  } else {
  ...
  if (existsSync(resourcesDir) && !existsSync(appFolderPath)) {
    ...
    if (tarZstFile) {
      ...
      try {
        ...
        // Tar contains PdfStudio/... — extract to parent of buildSourceDir so contents merge
