#!/usr/bin/env bash
# Create PdfStudio wrapper app - AppleScript droplet that launches internal main app
set -euo pipefail
...
echo "[create-wrapper] Creating PdfStudio wrapper app..."

# Find the main app bundle (in subdirectories, not the wrapper we're about to create)
MAIN_APP=$(find "$BUILD_DIR" -name "*.app" -type d ! -path "*/PdfStudio.app/*" 2>/dev/null | head -1)
...
TMP_DIR=$(mktemp -d /tmp/pdfstudio_wrapper.XXXXXX)
...
on open theFiles
    ...
    do shell script "mkdir -p /tmp/pdfstudio && echo " & quoted form of filePath & " > /tmp/pdfstudio/pending.txt"
...
# Wrapper app name
WRAPPER_APP="$BUILD_DIR/PdfStudio.app"
rm -rf "$WRAPPER_APP"
...
<dict>
    ...
    <string>com.drewjya.pdfstudio_launcher</string>
